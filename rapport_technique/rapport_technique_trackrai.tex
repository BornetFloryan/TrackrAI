...
Enfin, un module d'intelligence artificielle produit un score prédictif de séance à partir de statistiques calculées sur l'historique. L'objectif fonctionnel est de tendre vers un suivi qui s'adapte progressivement au sportif au fur et à mesure des séances enregistrées. Dans l'état actuel, cette adaptation est réalisée par un modèle XGBoost ré-entraînable sur les séances disponibles, avec une augmentation synthétique contrôlée afin de stabiliser la démonstration lorsque le volume de données réel reste limité.
...
Le module IA prédit un score global de séance à partir de statistiques calculées. Il ne remplace pas le score déterministe, mais fournit une estimation entraînée sur l'historique disponible. L'objectif produit est que l'évaluation devienne progressivement plus représentative du sportif suivi lorsque de nouvelles séances sont ajoutées à la base. Le système actuel met en place cette base technique avec un modèle ré-entraînable et des conseils simples de récupération ; il ne constitue pas encore un générateur autonome de programmes d'entraînement personnalisés.
...
Le dataset réel reste réduit. L'augmentation synthétique améliore la stabilité de la démonstration, mais un modèle sportif généraliste demanderait davantage de séances réelles, davantage d'athlètes et des annotations fiables. Le système actuel exploite l'historique pour prédire et expliquer une séance ; la personnalisation complète de l'entraînement reste une perspective. Une extension possible est l'intégration d'un assistant LLM de génération de programmes personnalisés utilisant l'historique des séances, la fatigue, la récupération, les objectifs et les résultats vidéo.
