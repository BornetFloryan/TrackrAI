...
Le contexte principal d'utilisation est le suivi d'une séance d'entraînement réalisée par un sportif. Une séance peut être démarrée depuis l'interface applicative, puis alimentée par un module connecté qui transmet des mesures GPS, inertielles et cardiaques. Les données sont ensuite agrégées, filtrées et transformées en statistiques de séance. Le sportif consulte son historique, ses scores et son coach référent. Le coach consulte les sportifs qui lui sont affectés et peut comparer leurs séances. L'administrateur gère les utilisateurs, les modules et les affectations coach/sportif.

L'application propose également une analyse vidéo de mouvements sportifs. Le flux vidéo est envoyé à un serveur Python par WebSocket, analysé avec MediaPipe/OpenCV, puis stocké dans l'historique. Le squat constitue le mode d'analyse principal. Des modes expérimentaux existent pour le développé couché et le soulevé de terre.
...
Module matériel & ESP32, GPS, MPU6050, BLE HR & Acquiert les mesures physiques et les transmet au serveur central pendant une séance active. \\
\bottomrule
\end{longtable}
...
\texttt{docs/} & Documentation de développement, tests et limites techniques. \\
\texttt{deliverables/} & Checklist technique de rendu. \\
\bottomrule
\end{longtable}
...
\midrule
\texttt{views/} & Pages principales : tableau de bord sportif, historique, détail séance, administration, coach, analyse vidéo. \\
\texttt{components/} & Composants réutilisables : navigation, graphiques, cartes statistiques. \\
\texttt{services/api.service.js} & Client Axios, ajout de l'access token, refresh automatique sur réponse 401. \\
...
\begin{itemize}
  \item \textbf{sportif} : consultation du tableau de bord, lancement de séance, historique, analyse vidéo ;
  \item \textbf{coach} : consultation des sportifs suivis, comparaison, accès aux analyses du périmètre affecté ;
...
\begin{itemize}
  \item un access token court, envoyé dans l'en-tête \texttt{x-session-id} ;
  \item un refresh token long, utilisé uniquement sur \texttt{POST /auth/refresh}.
\end{itemize}
...
\begin{itemize}
  \item \texttt{POST /auth/signin} vérifie le mot de passe avec bcrypt et crée les deux jetons ;
  \item l'access token est stocké en base dans le champ \texttt{sessionId} ;
  \item le refresh token est stocké en base dans le champ \texttt{refreshToken} ;
  \item chaque jeton possède une date d'expiration ;
  \item \texttt{POST /auth/refresh} remplace les deux jetons, ce qui invalide l'ancien refresh token ;
  \item \texttt{POST /auth/logout} supprime les deux jetons en base.
\end{itemize}
...
Une fois connecté au serveur central, le module envoie \texttt{AUTOREGISTER} si nécessaire, puis \texttt{HELLO}. Les mesures ne sont envoyées que lorsque le serveur central transmet une commande de démarrage de séance. Les données GPS sont filtrées côté module avec une distance minimale, une vitesse minimale et un intervalle minimal entre deux positions. Les données d'accéléromètre sont envoyées après une phase de stabilisation du MPU6050. La fréquence d'envoi est d'environ une seconde.
...
\texttt{physio.js} & Estimation des pas à partir de l'accélération, calcul de stress, qualité du comptage de pas. \\
\texttt{sessionStats.js} & Agrégation globale d'une séance : distance, vitesse, fréquence cardiaque, pas, stress, score et qualité. \\
\texttt{performanceScore.js} & Calcul du score de performance de référence. \\
\bottomrule
\end{longtable}

Les mesures GPS et accéléromètre restent dépendantes du matériel et de son placement. Pour cette raison, la séance contient également des indicateurs de qualité qui signalent si la trace GPS ou le comptage de pas sont exploitables, partiellement bruités ou peu fiables.
...
Après analyse, le résultat est renvoyé au client puis transmis au serveur central avec \texttt{STOREANALYSIS}. Le serveur central le relaie ensuite vers l'API, qui le stocke dans la collection \texttt{measures} avec un \texttt{analysisId}.
...
Le système d'authentification utilise deux jetons. L'access token est court et transmis dans les requêtes courantes. Le refresh token est plus long et permet d'obtenir un nouvel access token sans nouvelle saisie du mot de passe. Lors d'un refresh, le refresh token est également renouvelé, ce qui invalide l'ancien jeton. La déconnexion supprime les deux jetons côté serveur.
...
\begin{enumerate}
  ...
  \item connexion coach et vérification du filtre \og mes sportifs \fg{} ;
  \item connexion sportif et vérification du coach référent ;
  \item démarrage et arrêt d'une séance avec un module ESP32 ;
  \item upload d'une vidéo et consultation de l'analyse dans l'historique ;
  \item vérification des indicateurs de qualité GPS/pas sur une nouvelle séance.
\end{enumerate}
...
Le GPS et l'accéléromètre restent sensibles au matériel, à l'environnement et au placement du module. Les filtres appliqués réduisent les points incohérents et fournissent un indicateur de qualité, mais ils ne transforment pas le module en dispositif de mesure scientifique.
